...
canvas.height =
  window.innerHeight - (infoDiv.clientHeight + footerDiv.clientHeight);
// array to hold all the bubble objects.
let bubbles = [];
// initialize all necessary variables.
let variables = {
  text: "TINKER",
  colorPalette: ["#081226", "#011C40", "#1F4C73", "#3A6D8C", "#689BA6"],
  minRadius: 3,
  maxRadius: 7,
  minOpacity: 0.4,
  maxOpacity: 0.9,
  ...
  if (canvas.width < 500) {
    ...
    for (let i = 0; i < userTextSplit.length; i++) {
      ...
      ctx.fillText(
        userTextSplit[i],
        canvas.width / 2,
        canvas.height / 4 + (i * canvas.width) / 4
      );
    }
  } else {
    ctx.font = `lighter ${canvas.width / 9}px Verdana`;
    ctx.fillStyle = "#f2f2f2";
    ctx.textAlign = "center";
